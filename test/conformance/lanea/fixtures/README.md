...
| `inventory/host-packages.json` | `internal/collector/host`'s `Package` JSON tags | A host inventory for a machine that has no dpkg, rpm or apk. **Note what is missing: there is no purl field, because `host.Package` has none.** That absence is SEAM 2. |
